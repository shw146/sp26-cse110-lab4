...
17. The function will return the array [2, 4, 6]. The call to modifyArray() will jump to the function declaration in line 1 where it then registers doSomething as a callback and continues on. modifyArray iterations through all of the elements of the inputted array from start to end and calls doSomething on them before pushing them onto a new array that is initially empty. This would keep the same order of values, even after the translation done by doSomething.
19. The code should output 1342, notably because the delay on 2 is significantly slow. On a slower computer with a lesser delay, 2 *may* happen before 4, but that would require some really slow computation and a context switch before the system could print out 4.
